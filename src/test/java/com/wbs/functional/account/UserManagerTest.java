package com.wbs.functional.account;
...
import com.wbs.functional.BaseFunctionalTestCase;
import com.wbs.functional.Gui;
import com.wbs.functional.Gui.UserColumn;
...
public class UserManagerTest extends BaseFunctionalTestCase {
	...
	public void overviewPage() {
		...
		WebElement table = driver.findElement(By
				.xpath("//table[@id='contentTable']"));
		assertEquals(
				"admin",
				SeleniumUtils.getTable(table, 1,
						UserColumn.LOGIN_NAME.ordinal()));
		assertEquals("User",
				SeleniumUtils.getTable(table, 1, UserColumn.NAME.ordinal()));
		assertEquals("管理员, 用户",
				SeleniumUtils.getTable(table, 1, UserColumn.ROLES.ordinal()));
	}
	...
	public void createUser() {
		...
		driver.findElement(By.id("checkedRoleIds-" + role.getId()))
				.setSelected();
	...
	public void editUser() {
		...
		SeleniumUtils.type(driver.findElement(By.id("name")),
				testUser.getName());

		// 取消所有角色
		Role role = testUser.getRole();
		SeleniumUtils.uncheck(driver.findElement(By.id("checkedRoleIds-"
				+ role.getId())));
		testUser.setRole(null);

		// 增加一个角色
		role = AccountData.getRandomDefaultRole();
		driver.findElement(By.id("checkedRoleIds-" + role.getId()))
				.setSelected();
	...
	public void deleteUser() {
		...
		assertFalse(StringUtils.contains(
				driver.findElement(By.id("contentTable")).getText(),
	...
	private void searchUser(String loginName) {
		SeleniumUtils.type(driver.findElement(By.name("filter_EQS_loginName")),
				loginName);
	...
	private void verifyUser(User user) {
		...
		assertEquals(user.getLoginName(), driver
				.findElement(By.id("loginName")).getValue());
		assertEquals(user.getName(), driver.findElement(By.id("name"))
				.getValue());

		Role role = user.getRole();
		assertTrue(driver.findElement(By.id("checkedRoleIds-" + role.getId()))
				.isSelected());

		List<Role> userRoles = new ArrayList<Role>();
		userRoles.add(user.getRole());
		List<Role> uncheckRoleList = ListUtils.subtract(
				AccountData.getDefaultRoleList(), userRoles);
		for (Role unRole : uncheckRoleList) {
			assertFalse(driver.findElement(
					By.id("checkedRoleIds-" + unRole.getId())).isSelected());
		}
	}
