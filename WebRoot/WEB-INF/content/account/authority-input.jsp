...
<head>
	<title>${appTitle} 系统权限管理-保存</title>
...
<form id="inputForm" action="#" method="post" enctype="multipart/form-data">
    ...
    <tr>
    	<th height="25" class="ui-widget-header ui-corner-all" colspan="3"><b><c:out value="${_create? '增加':'修改'}" />系统权限</b></th>
    </tr>
    <tr>
		<td width="30%" height="23" class="ui-state-hover" align="right">			
			权限名：<b class="requiredStar">*</b>
		</td>
    ...
    <tr>
      <td height="23" class="ui-widget-header" align="center" colspan="3">
...
function initPageUI() {
	$(window).initInputPageUI({
		saveAction: '${ctx}/account/authority!save.action',
		validations: {
			onfocusout: false,
			rules: {
				name: {remoteValid: {url:"${ctx}/account/authority!checkName.action", data:{oldName:'${name}'}}}
			},
			messages: {
				name: {remoteValid: "该权限名已被使用!"}
			}
		}
	});
}
