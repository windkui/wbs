...
<head>
	<title>${appTitle} 系统菜单管理-列表</title>
...
<form action="${ctx}/account/menu!delete.action" method="post" id="listForm">
	...
	<dataTable:header orderableValues="true,true,true,true,false" 
			fieldValues="id,name,command,parent,-"
			labelValues="编号,菜单名,菜单命令,上级菜单,操作"></dataTable:header>
	</thead>
	<tbody>
	...
	<tr>
		...
		<td class="ui-state-default" align="left">
			<div class="ui-button ui-state-active ui-corner-all ui-button-text-only">
				<span class="ui-button-text">${dto.command}</span>
			</div>
		</td>				
		<td class="ui-state-default" width="20%" align="left">
			<div class="ui-button ui-state-active ui-corner-all ui-button-text-only">
				<span class="ui-button-text">${dto.parent.name}</span>
			</div>
		</td>
		<td class="ui-state-default" align="center" width="120">
			<div class="ui-buttonset">
     		<a class="edit-button" style="padding: 0px;" href="${ctx}/account/menu!input.action?id=${dto.id}" title="编辑">
     			<img src="${ctx}/images/icon_edit.gif" border="0" width="16" height="16" />
     		</a>
     		<a class="delete-button" style="padding: 0px;" href="${ctx}/account/menu!delete.action?id=${dto.id}" title="删除">
