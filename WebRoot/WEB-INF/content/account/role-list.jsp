...
<head>
	<title>${appTitle} 系统角色管理-列表</title>
...
<form action="${ctx}/account/role!delete.action" method="post" id="listForm">
	...
	<dataTable:header orderableValues="true,true,true,true,false" 
			fieldValues="id,name,authorityList,menuList,-"
			labelValues="编号,角色名,角色权限,角色菜单,操作"></dataTable:header>
	</thead>
	<tbody>
	...
	<tr>
		...
		<td class="ui-state-default" align="left">
			<div class="ui-button ui-state-active ui-corner-all ui-button-text-only">
				<span class="ui-button-text">${dto.authNames}</span>
			</div>
		</td>				
		<td class="ui-state-default" width="30%" align="left">
			<div class="ui-button ui-state-active ui-corner-all ui-button-text-only">
				<span class="ui-button-text">${dto.menuNames}</span>
			</div>
		</td>
		<td class="ui-state-default" align="center" width="120">
			<div class="ui-buttonset">
     		<a class="edit-button" style="padding: 0px;" href="${ctx}/account/role!input.action?id=${dto.id}" title="编辑">
     			<img src="${ctx}/images/icon_edit.gif" border="0" width="16" height="16" />
     		</a>
     		<a class="delete-button" style="padding: 0px;" href="${ctx}/account/role!delete.action?id=${dto.id}" title="删除">
