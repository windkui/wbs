...
<div class="ui-widget-header ui-corner-all" style="padding: 2px;">
   <button id="refresh-button">系统菜单管理</button>
   <button id="toolbar-button">工具栏</button>   
</div>
<div id="toolbarPanel" class="ui-corner-all ui-state-default">
	<form id="searchForm" name="searchForm" action="${ctx}/account/menu!list.action" method="post">	
		...
		<tr>
			...
			<td class="ui-state-hover ui-corner-all" colspan="6">
				...
				<button type="button" id="add-button" value="${ctx}/account/menu!input.action">添加</button>
			</td>
		</tr>	
		</tbody>
		</table>
...
function initPageUI() {
	jQuery(window).initDataPagerUI({
		gridConfig: {
			limit: 10,
			colModel: [
				{header:'菜单名', name: 'name', width: 100, sort:'clientSide'},
				{header:'菜单命令', name: 'command', width: 300, sort:'clientSide'},
				{header:'上级菜单', name: 'parentName', width: 200, wrap:true, sort:'clientSide'},
